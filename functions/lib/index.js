...
if (!GITHUB_CLIENT_ID || !GITHUB_CLIENT_SECRET) {
    console.error("錯誤：請設定 GITHUB_CLIENT_ID 和 GITHUB_CLIENT_SECRET 環境變數");
    process.exit(1);
}
    ...
    try {
        ...
        if (!repoExists) {
            ...
            try {
                ...
                const finalDescription = currentDescription.includes("Deployed by EZPage")
                    ? currentDescription
                    : description
                        ? `${description} - Deployed by EZPage`
...
app.get("/repositories", async (req, res) => {
    try {
        ...
        const ezpageRepos = repos.filter((repo) => repo.description && repo.description.includes("Deployed by EZPage"));
